        ...
        <? if (isset($type) && $type) { ?>
          <div class="panel panel-default">
            <div class="panel-heading">
                <? $status = ParcelsStatuses::model()->find('value=:value', [':value' => $type]); ?>
            ...
            $this->widget(
              ...
              [
                ...
                'summaryText'      => Yii::t('main', 'Посылки') . ' {start}-{end} ' . Yii::t(
                    'main',
                    'из'
                  ) . ' {count}',
                'htmlOptions'      => ['style' => 'cursor: pointer;'],
                'selectionChanged' => "function(id){window.location='"
                  . Yii::app()->urlManager->createUrl('/cabinet/parcels/view') . "/id/' + 
          $.fn.yiiGridView.getSelection(id);}",
                'columns'          => [
                  [
                    'name'  => 'id',
                    'type'  => 'raw',
                    'value' => 'CHtml::link(CHtml::encode($data->uid."-".$data->id),array("/cabinet/parcels/view", "id"=>$data->id))',
                  ],
                  [
                    'header'      => Yii::t('main', 'Товары'),
                    'type'        => 'raw',
                    'value'       => 'Parcels::getParcelItemsPreview($data->id,"_60x60.jpg")',
