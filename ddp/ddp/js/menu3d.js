(function ($) {
    ...
    var title, lazyImages, responsive, clickable, skin, speed, vertical, easing, hoverIn, hoverOut, hoverInTimeout, hoverOutTimeout;
    var lastMenu, heightOfMenu, widthOfMenu, topOfMenu, leftOfMenu, rightOfMenu, bottomOfMenu, hoverInTimeoutFunction,
        hoverOutTimeoutFunction;
    ...
    $.fn.menu3d = function (options) {
        ...
        hoverInTimeout = opts.hoverInTimeout;
        hoverOutTimeout = opts.hoverOutTimeout;
    ...
    $.fn.menu3d.lazyLoadImages = function (li) {
        if (lazyImages) {
            try {
                var lazyImagesList;
                lazyImagesList = $(li).find('img[data-src]'); //
                if (typeof lazyImagesList !== 'undefined') {
                    $.each(lazyImagesList,
                        function (i, lazyImg) {
                ...
                console.log('Problems with menu lazyImages');
            }
        }
    };
    $.fn.menu3d.init = function (obj) {
        $this = $(obj);
        $this.prepend('<div class="menuToggle"> ' + title + ' <span style="float: right;">&#9776;</span></div>');
        $this.find('.txtSearch').focus(function () {
            $(this).val('');
        }).blur(function () {
            $(this).val('Search...');
        });
    ...
    $.fn.menu3d.hoverIn = function (li) {
        if (lastMenu == li) {
            clearTimeout(hoverOutTimeoutFunction);
        }
        lastMenu = li;
        hoverInTimeoutFunction = setTimeout(function () {
            $.fn.menu3d.lazyLoadImages(li);
    ...
    $.fn.menu3d.hoverOut = function (li) {
        clearTimeout(hoverInTimeoutFunction);
        hoverOutTimeoutFunction = setTimeout(function () {
            if ($(li).length > 0) {
                var div = $(li).find('> div');
                var hoverInOfMenu = hoverIn;
                var hoverOutOfMenu = hoverOut;
