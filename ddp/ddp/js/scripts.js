...
//sidebar accordion
        $('.page-sidebar .collapse').on('shown.bs.collapse', function () {
            $(this).parent().find('.fa-arrow-right').removeClass('fa-arrow-right').addClass('fa-arrow-down');
            $(this).parent().find('.panel-heading').removeClass('closed').addClass('opened');
        }).on('hidden.bs.collapse', function () {
            $(this).parent().find('.fa-arrow-down').removeClass('fa-arrow-down').addClass('fa-arrow-right');
            $(this).parent().find('.panel-heading').removeClass('opened').addClass('closed');
        });

        /*
$('#filter-filters .panel-heading a').on('click',function(e){
...
//quantity script
        ...
        $('.qtyplus').click(function () {
            $(this).parent().parent().find(':text[name^=\'num\']').val(Number($(this).parent().parent().find(':text[name^=\'num\']').val()) + 1);
        });

        $('.qtyminus').click(function () {
            if ($(this).parent().parent().find(':text[name^=\'num\']').val() > 1)
                $(this).parent().parent().find(':text[name^=\'num\']').val(Number($(this).parent().parent().find(':text[name^=\'num\']').val()) - 1);

        });
        $('.qtyplus').click(function () {
            $(this).parent().parent().find(':text[name^=\'weight\']').val(Number($(this).parent().parent().find(':text[name^=\'weight\']').val()) + 50);
        });

        $('.qtyminus').click(function () {
            if ($(this).parent().parent().find(':text[name^=\'weight\']').val() > 50)
                $(this).parent().parent().find(':text[name^=\'weight\']').val(Number($(this).parent().parent().find(':text[name^=\'weight\']').val()) - 50);
...
(function ($) {
    ...
    $.fn.validateOnBlur = function (language, settings) {
        ...
        this.find('input[data-validation], textarea[data-validation]').unbind('blur', blurValidationCallback).bind('blur', blurValidationCallback);
        return this;
    };
    $.fn.showHelpOnFocus = function (attrName) {
        ...
        this.find('.has-help-txt').valAttr('has-keyup-event', false).valAttr('backend-valid', false).valAttr('backend-invalid', false).unbind('focus').unbind('blur').removeClass('has-help-txt');
        this.find('textarea,input').each(function () {
            var $element = $(this), className = 'jquery_form_help_' + $element.attr('name'), help = $element.attr(attrName);
            if (help) {
                $element.addClass('has-help-txt').focus(function () {
    ...
    $.fn.validateForm = function (language, config) {
        ...
        if (!$.formUtils.haltValidation && errorInputs.length > 0) {
            ...
            if (config.errorMessagePosition === 'top') {
                ...
                $.each(errorInputs, function (i, $input) {
                    var $parent = $input.parent(), $errorSpan = $parent.find('span[class=' + config.errorMessageClass + ']');
    ...
    $.formUtils = {
        ...
        validateInput: function ($element, language, config, $form, eventContext) {
            var value = $.trim($element.val() || ''), optional = $element.valAttr('optional'), validationDependsOnCheckedInput = false,
        ...
        parseDate: function (val, dateFormat) {
            var divider = dateFormat.replace(/[a-zA-Z]/gi, '').substring(0, 1), regexp = '^', formatParts = dateFormat.split(divider),
        ...
        name: 'number', validatorFunction: function (val, $el, config) {
            if (val !== '') {
                var allowing = $el.valAttr('allowing') || '',
                    decimalSeparator = $el.valAttr('decimal-separator') || config.decimalSeparator, allowsRange = false, begin, end;
